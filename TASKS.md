...
- **ملاحظة مهمة:** نفكرو أن هاد ال-features **مستمرة** → باش نخدمو مع BernoulliNB خاص binarization. سجّل thresholds في `experiments/bernoulli/config.yaml` أو `bin_thresholds.yaml`.
...
- **ملاحظة على binarization:** Hu features هم continuous → لازم binarize حسب strategy (global median أو Otsu).
...
- **Outputs:**  
  ...
  - `experiments/bernoulli/metrics.csv` (accuracy, balanced_accuracy, macro_f1, per-class precision/recall)  
- **ملاحظة تقنية:** حدّد طريقة binarization بـconfig (مثال: `--binarize median` أو `--binarize quantile 0.5`).
...
- Mohamed يبدى الآن بـ**A1 (prepare_all.py)** و**A3 (PCA extraction)** على 10–20 sample كنموذج اختبار.  
- Imad يقدر يبدا B1 (render_views) على نفس الـ10–20 sample أو يستعمل few raw pcl لبدء استخراج Hu Moments.  
